...
from apps.produccion.forms                  import UnidadProduccionForm
...
class AdminUnidadProduccion(admin.ModelAdmin):
    
    inlines = [AdminUnidadResponsableInline,]
     # Accesos directos del lado derecho
    def ver(self, obj):
        return format_html('<a class="btn btn-primary btn-sm" href="/admin/produccion/unidad_productiva/{}/change/"><i class="nav-icon fas fa-eye" title="Ver detalles"></i></a>', obj.id)
    
    def eliminar(self, obj):
        return format_html('<a class="btn btn-danger btn-sm" href="/admin/produccion/unidad_productiva/{}/delete/"><i class="nav-icon fas fa-trash"></i></a>', obj.id)
    ...
    list_display        = ('nombre','estado','municipio','parroquia','tipos_establecimiento','razon_social','ente_encomienda','ver','eliminar')
    #list_filter         = ('motivo',)
    search_fields       = []
...
admin.site.register(UnidadProduccion, AdminUnidadProduccion)